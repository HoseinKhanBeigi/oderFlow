...
export async function fetchVenueKlines(
  ...
  if (exchange === 'binance') {
    ...
    const startQ = Number.isFinite(startTime) ? `&startTime=${startTime}` : '';
    const endQ = Number.isFinite(endTime) ? `&endTime=${endTime}` : '';
    const data = await fetchJson(
      `${base}?symbol=${encodeURIComponent(inst)}&interval=${encodeURIComponent(binanceInterval(interval))}&limit=${Math.min(limit, 1500)}${startQ}${endQ}`,
    );
